...
TEST_CASE("SSE Base: sml::VectorSet & Equality Check", "[sml::VectorSet] [IsZero] [sml::VectorIsEqual]") {
	//Using the namespace sml (SSE Math Library)
	using namespace sml;
	
	SECTION("sml::VectorSet & sml::VectorIsEqual", "[sml::VectorSet] [Equal]") {
		float trueRandA[4] = { rand_num(), rand_num(), rand_num(), rand_num() };
		__m128 trueRandM128 = sml::VectorSet(trueRandA);
		__m128 randM128Set = sml::VectorSet(79553.85881f, 97658.71592f, 23392.63518f, 1572.67638f);
		__m128 trueM128Set = _mm_shuffle_ps(randM128A, randM128A, 0x1B); //0x1B = 00 01 10 11, or 0 1 2 3.

		__m128 Dupe = sml::VectorSet(randFPB);
		const float* DupeFP = randFPA;

		CHECK(sml::VectorIsEqual(trueRandA, trueRandM128));
		CHECK_FALSE(sml::VectorIsEqual(randM128Set, randM128A));
		CHECK(sml::VectorIsEqual(randM128Set, trueM128Set));
		CHECK_FALSE(sml::VectorIsEqual(randM128A, randM128B));
		CHECK(sml::VectorIsEqual(randFPB, Dupe));
		CHECK(sml::VectorIsEqual(DupeFP, randFPA));
	}
}

TEST_CASE("SSE Base: Basic Vector Math", "[sml::VectorAdd], [sml::VectorSub], [sml::VectorMul], [sml::VectorDiv]") {
	//Using the namespace sml (SSE Math Library)
	using namespace sml;
	...
	SECTION("Addition", "[sml::VectorAdd]") {
		answer[0] = sml::VectorSet(97583.98391f, 136005.67413f, 42062.23443f, 46888.12591f);
		answer[1] = sml::VectorSet(97738.86711f, 97929.25696f, 130262.03084f, 110733.25232f);
		answer[2] = sml::VectorSet(100692.11219f, 138626.95151f, 72643.668350f, 31298.989940f);
		answer[3] = sml::VectorSet(94630.73883f, 95307.97958f, 99680.59692f, 126322.38829f);

		__m128 myVec;
		myVec = sml::VectorAdd(randFPA, randFPB);
		CHECK(sml::VectorIsEqual(myVec, answer[0]));

		//Static sml::VectorSub: m128 & m128
		myVec = sml::VectorAdd(randM128A, randM128B);
		CHECK(sml::VectorIsEqual(myVec, answer[1]));

		//Static sml::VectorSub: Float Pointer & m128
		myVec = sml::VectorAdd(randFPA, randM128B);
		CHECK(sml::VectorIsEqual(myVec, answer[2]));

		//Static sml::VectorSub: m128 & Float Pointer
		myVec = sml::VectorAdd(randM128A, randFPB);
		CHECK(sml::VectorIsEqual(myVec, answer[3]));
	}

	SECTION("Subtraction", "[sml::VectorSub]") {
		answer[0] = sml::VectorSet(-88532.14099f, -7825.01467f, 38018.47243f, -46648.93305f);
		answer[1] = sml::VectorSet(-94593.51435f, -51143.9866f, 65055.40100f, 48374.46530f);
		answer[2] = sml::VectorSet(-91640.26927f, -10446.29205f, 7437.038510f, -31059.79708f);
		answer[3] = sml::VectorSet(-91485.38607f, -48522.70922f, 95636.834920f, 32785.32933f);
	...
	SECTION("Scalar Multiply", "[sml::VectorSub]") {
		answer[0] = sml::VectorSet(12898.8761610f, 182657.4397305f, 114115.0072755f, 340.8498255f);
		answer[1] = sml::VectorSet(683046.1783830f, 527858.6278960f, 14840.6065400f, 343281.0063832f);
		answer[2] = sml::VectorSet(4482.1276830f, 66669.0102630f, 278327.3403720f, 226728.4976085f);
		answer[3] = sml::VectorSet(705859.8399582f, 547098.8038652f, 239308.3315128f, 228856.7483634f);
	...
	SECTION("Scalar Divide", "[sml::VectorDiv]") {
		answer[0] = sml::VectorSet(1588.0426175f, 22487.8349929f, 14049.2468175f, 41.9636596f);
		answer[1] = sml::VectorSet(551.8162736f, 8207.9421684f, 34266.2161122f, 27913.6346701f);
	...
	SECTION("sml::VectorNegate", "[sml::VectorNegate]") {
		answer[0] = sml::VectorSet(-4525.92146f, -64090.32973f, -40040.35343f, -119.59643f);
		answer[1] = sml::VectorSet(-1572.67638, -23392.63518f, -97658.71592f, -79553.85881f);
...
TEST_CASE("SSE Base: Basic Normal Math Functions", "[Min] [Max] [Average] [Length] [LengthSq]") {
	...
	SECTION("Minimum Value (Per Component)", "[Min]") {
		answer[0] = sml::VectorSet(4525.92146f, 64090.32973f, 2021.88100f, 119.59643f);
		answer[1] = sml::VectorSet(1572.67638f, 23392.63518f, 32603.31492f, 31179.39351f);
		answer[2] = sml::VectorSet(4525.92146f, 64090.32973f, 32603.31492f, 119.59643f);
		answer[3] = sml::VectorSet(1572.67638f, 23392.63518f, 2021.88100f, 46768.52948f);
	...
	SECTION("Maximum Value (Per Component)", "[Max]") {
		answer[0] = sml::VectorSet(93058.06245f, 71915.34440f, 40040.35343f, 46768.52948f);
		answer[1] = sml::VectorSet(96166.19073f, 74536.62178f, 97658.71592f, 79553.85881f);
		answer[2] = sml::VectorSet(96166.19073f, 74536.62178f, 40040.35343f, 31179.39351f);
		answer[3] = sml::VectorSet(93058.06245f, 71915.34440f, 97658.71592f, 79553.85881f);
	...
	SECTION("Maximum Value (Per Component)", "[Max]") {
		answer[0] = sml::VectorSet(48791.991955f, 68002.837065f, 21031.117215f, 23444.062955f);
		answer[1] = sml::VectorSet(48869.433555f, 48964.628480f, 65131.015420f, 55366.626160f);
		answer[2] = sml::VectorSet(50346.056095f, 69313.475755f, 36321.834175f, 15649.494970f);
		answer[3] = sml::VectorSet(47315.369415f, 47653.989790f, 49840.298460f, 63161.194145f);
...
TEST_CASE("SSE Base: Vector Math Functions", "[Dot], [Cross], [Normalize], [Homogenize], [AngleBetween], [Component], [Project], [Reflect]") {
	...
	SECTION("Cross Product Test", "[Cross]") {
		answer[0] = sml::VectorSet(-0.308812887f, 0.417404713f, -0.633209766f, 0.0f);
		answer[1] = sml::VectorSet(-0.515099653f, 0.738302691f, -0.168553930f, 0.0f);
		answer[2] = sml::VectorSet(-0.093845581f, 0.388312880f, -0.610942729f, 0.0f);
		answer[3] = sml::VectorSet(-0.590497562f, 0.769012140f, -0.174695711f, 0.0f);

		union { float f; unsigned int i = 0xFFFFFFFF; } ufi;

		__m128 randFPA_3D = sml::VectorSet(randFPA);
		randFPA_3D = _mm_and_ps(randFPA_3D, _mm_set_ps(0.0f, ufi.f, ufi.f, ufi.f));
		randFPA_3D = Normalize(randFPA_3D);

		__m128 randFPB_3D = sml::VectorSet(randFPB);
	...
	SECTION("Normalize Test", "[Normalize]") {
		answer[0] = sml::VectorSet(0.059783384f, 0.846576074f, 0.528897344f, 0.001579762f);
		answer[1] = sml::VectorSet(0.012274654f, 0.182578253f, 0.762221000f, 0.620913569f);
	...
	SECTION("Homogenize Test", "[Homogenize]") {
		answer[0] = sml::VectorSet(1.989758144f, 1.537686670f, 0.043231656f, 1.0f);
		answer[1] = sml::VectorSet(3.084286764f, 2.390573176f, 1.045668669f, 1.0f);
	...
	SECTION("Project Test", "[Project]") {
		answer[0] = sml::VectorSet(29717.254507533f, 22965.517831192f, 645.66949578634f, 14935.108865467f);
		answer[1] = sml::VectorSet(43171.054190119f, 33461.079341774f, 14636.323478182f, 13997.094785780f);
		answer[2] = sml::VectorSet(37244.234502319f, 28867.311884893f, 12626.948173945f, 12075.477199538f);
		answer[3] = sml::VectorSet(33375.627580646f, 25792.711440966f, 725.15530080633f, 16773.710749218f);
	...
	SECTION("Reflect Test", "[Reflect]") {
		answer[0] = sml::VectorSet(0.848413913f, -0.144721481f, -0.509165517f, 0.0f);
		answer[1] = sml::VectorSet(0.597331600f, 0.242201118f, -0.764547956f, 0.0f);
		answer[2] = sml::VectorSet(0.983832565f, -0.037689804f, -0.175079873f, 0.0f);
		answer[3] = sml::VectorSet(0.255711896f, -0.023200824f, -0.966474597f, 0.0f);
