...
TEST_CASE("Basic Vector Math", "[Add], [Sub], [Mul], [Div]") {
	...
	SECTION("Addition", "[Add], [operator+], [operator+=]") {
		...
		answer[14] = vec4::Set( 94630.73883f,  95307.97958f,  99680.59692f, 126322.38829f);
	...
	SECTION("Subtraction", "[Sub], [operator-], [operator-=]") {
		...
		answer[14] = vec4::Set( -91485.38607f,  -48522.70922f,  95636.834920f,   32785.32933f);
	...
	SECTION("Scalar Divide", "[Div], [operator/=], [operator/]") {
		...
		answer[2] = vec4::Set( 6808.0181052f, 33320.0363122f,   111.0872140f, 30467.4838245f);
...
TEST_CASE("Basic Normal Math Functions ", "[Min], [Max], [Average],[Length], [LengthSq]") {
	...
	SECTION("Maximum Value [Per Component]", "[Max]") {
		...
		answer[7] = vec4::Set(96166.19073f, 74536.62178f, 97658.71592f, 79553.85881f);
	...
	SECTION("Average Value [Per Component]", "[Avg]") {
		...
		answer[9] = vec4::Set(47315.369415f, 47653.989790f, 49840.298460f, 63161.194145f);
...
TEST_CASE("Vector Math Functions", "[Dot], [Cross], [Normalize], [Homogenize], [AngleBetween], [Component], [Project], [Reflect]") {
	...
	SECTION("Normalize Test", "[Normalize]") {
		//Setup Answers for this section
		answer[0] = vec4::Set(0.149101802f, 0.729739170f, 0.002432911f, 0.667265670f);
		answer[1] = vec4::Set(0.393810990f, 0.518324295f, 0.759082568f, 0.006817782f);
		answer[2] = vec4::Set(0.059783384f, 0.846576074f, 0.528897344f, 0.001579762f);
		answer[3] = vec4::Set(0.012274654f, 0.182578253f, 0.762221000f, 0.620913569f);
	...
	SECTION("Project Test", "[Project]") {
		//Setup Answers for this section
		answer[0] =  vec4::Set(22720.747894474f, 29904.487003928f, 43794.927237608f,   393.348919536f);
		answer[1] =  vec4::Set( 2979.507544385f, 42191.987342330f, 26359.391381165f,    78.732799191f);
		answer[2] =  vec4::Set(  342.223246538f,  5090.369295423f, 21251.087153047f, 17311.368176471f);
		answer[3] =  vec4::Set(22720.747894474f, 29904.487003928f, 43794.927237608f,   393.348919536f);
		answer[4] =  vec4::Set(73738.377916177f, 56985.076883461f,  1602.120454201f, 37058.965237247f);
		answer[5] =  vec4::Set(25753.932250504f, 33896.689310678f, 49641.481619864f,   445.860385917f);
		answer[6] =  vec4::Set(66600.241719226f, 51620.605847018f, 22579.543164151f, 21593.401263600f);
		answer[7] =  vec4::Set(34426.058224149f, 45310.727249911f, 66357.266143115f,   595.995029268f);
		answer[8] =  vec4::Set(29717.254507533f, 22965.517831192f, 645.66949578634f, 14935.108865467f);
		answer[9] =  vec4::Set(43171.054190119f, 33461.079341774f, 14636.323478182f, 13997.094785780f);
		answer[10] = vec4::Set(37244.234502319f, 28867.311884893f, 12626.948173945f, 12075.477199538f);
		answer[11] = vec4::Set(33375.627580646f, 25792.711440966f, 725.15530080633f, 16773.710749218f);
